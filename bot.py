...
async def on_message(ctx,event_name,event_date):
    if event_name and event_date:
        print(ctx)
        uID = discord_events.events.generate_event(os.getcwd(),event_name,event_date,ctx.message.author.id)
...
async def on_message(ctx,event_id,desc):
    if desc:
        x = discord_events.events.alter_description(os.getcwd(),desc,event_id,ctx.message.author.id)
        if x==1:
